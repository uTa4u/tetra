...
public class TooltipGetterArthropod implements ITooltipGetter {
    ...
    private static final IStatGetter LEVEL_GETTER = new StatGetterEffectLevel(ItemEffect.ARTHROPOD, 1);
    ...
    public String getTooltip(EntityPlayer player, ItemStack itemStack) {
        return I18n.format("stats.arthropod.tooltip",
                String.format("%.2f", LEVEL_GETTER.getValue(player, itemStack) * 2.5),
                String.format("%.2f", LEVEL_GETTER.getValue(player, itemStack) * 0.5));
    }
}
